...
async def get_comandas(
    ...
    status_filtro: Optional[int] = Query(None, description="Filtrar por status: 0=aberta, 1=fechada, 2=cancelada"),
    funcionario_id: Optional[int] = Query(None, description="Filtrar por funcionário"),
    cliente_id: Optional[int] = Query(None, description="Filtrar por cliente"),
    data_inicio: Optional[datetime] = Query(None, description="Filtrar por data inicial (YYYY-MM-DDTHH:MM:SS)"),
    data_fim: Optional[datetime] = Query(None, description="Filtrar por data final (YYYY-MM-DDTHH:MM:SS)"),
    ...
    try:
        ...
        for comanda_row, funcionario, cliente in results:
            comanda_response = ComandaResponse(
                ...
                data_hora=comanda_row.data_hora,
                status=comanda_row.status,
                cliente_id=comanda_row.cliente_id,
                funcionario_id=comanda_row.funcionario_id,
                funcionario=FuncionarioResponse(
                    id=funcionario.id,
                    nome=funcionario.nome,
                    matricula=funcionario.matricula,
...
async def update_comanda(id: int, comanda_data: ComandaUpdate, request: Request, db: AsyncSession = Depends(get_async_db), current_user: FuncionarioAuth = Depends(require_group([1]))):
    try:
        ...
        await AuditoriaService.registrar_acao(
            db=db, funcionario_id=current_user.id, acao="UPDATE", recurso="COMANDA", recurso_id=comanda.id,
...
async def delete_comanda(id: int, request: Request, db: AsyncSession = Depends(get_async_db), current_user: FuncionarioAuth = Depends(require_group([1]))):
    try:
        ...
        await AuditoriaService.registrar_acao(
            db=db, funcionario_id=current_user.id, acao="DELETE", recurso="COMANDA", recurso_id=id,
            dados_antigos=comanda, request=request,
        )
    except HTTPException:
        raise
    except Exception as e:
...
async def cancelar_comanda(
    ...
    try:
        ...
        await AuditoriaService.registrar_acao(
            db=db, funcionario_id=current_user.id, acao="CANCEL", recurso="COMANDA",
...
async def add_produto_to_comanda(
    ...
    try:
        ...
        await AuditoriaService.registrar_acao(
            db=db, funcionario_id=current_user.id, acao="CREATE", recurso="COMANDA_PRODUTO",
            recurso_id=new_comanda_produto.id, dados_novos=new_comanda_produto, request=request,
        )
        return ComandaProdutosResponse(
            id=new_comanda_produto.id,
            comanda_id=new_comanda_produto.comanda_id,
            funcionario_id=new_comanda_produto.funcionario_id,
            funcionario=FuncionarioResponse(
                id=funcionario.id,
                nome=funcionario.nome,
                matricula=funcionario.matricula,
            ...
            produto_id=new_comanda_produto.produto_id,
            ...
            quantidade=new_comanda_produto.quantidade,
            valor_unitario=float(new_comanda_produto.valor_unitario),
        )
    except HTTPException:
        raise
    except Exception as e:
...
async def update_comanda_produto(
    ...
    try:
        ...
        # Registrar auditoria de atualização
        await AuditoriaService.registrar_acao(
            db=db, funcionario_id=current_user.id, acao="UPDATE", recurso="COMANDA_PRODUTO",
        ...
        result = await db.execute(
            ...
            .outerjoin(ProdutoDB, ComandaProdutoDB.produto_id == ProdutoDB.id)
            .outerjoin(FuncionarioDB, ComandaProdutoDB.funcionario_id == FuncionarioDB.id)
            .where(ComandaProdutoDB.id == id)
        )
        row = result.first()
        cp, produto, funcionario = row
        return ComandaProdutosResponse(
            id=cp.id,
            comanda_id=cp.comanda_id,
...
async def remove_produto_from_comanda(id: int, request: Request, db: AsyncSession = Depends(get_async_db), current_user: FuncionarioAuth = Depends(require_group([1]))):
    try:
        ...
        # Registrar auditoria de exclusão
        await AuditoriaService.registrar_acao(
            db=db, funcionario_id=current_user.id, acao="DELETE", recurso="COMANDA_PRODUTO",
